...
BU**GS** & JA**GS** = "Gibbs Sampling" has become a confusing term because its meaning has diverged.
...
linear_model_code <- nimbleCode({
  ...
  sigma ~ dunif(min = 0, max = 100) # Sigma is uniform since i tneeds to be positive, also used as standard deviation rather than precision
  for(i in 1:N) {
    predicted_y[i] <- intercept + slope*x[i]
    y[i] ~ dnorm(predicted_y[i], sd = sigma) #y[i] is observed, but will also have a probabilistic value with it, log probability
  }
})
```
...
-   *The order of the code does not matter!* `nimble` will figure out the order of calculations from seeing what depends on what. (This is called a *declarative* language; most languages are *imperative*, meaning the code is an ordered sequence of commadns.)
...
(Internally, nimble will turn the second version into the first version.) Creates "lifted nodes" during the interpretation process

# Building a model object from the code

The model code is only that: code.

To turn code into an object, we need to provide some combination of:

-   `constants` (like `N` or possibly `x`) are required to make sense of the
-   `data` (like `y` or possibly `x`). Can give the data later
...
linear_model <- nimbleModel(linear_model_code,
                            constants = list(N = 10), #Define the constant that is used in the nimbleCode
                            data = list(y = y, x = x)) 
linear_model$y
```

Note: Can choose not to provide data here, but will need to set the data at some point either using `model$y <- data$y` or `model$setdata(y = data$y, x = data$x)`. The setData **method** is applicable to any Nimble Model.
...
Setup: Ensure that the grouping variable is identified. Groups need to be integers with indexing starting at 1. Grouping data needs to be in the constants section (usually). If grouping data are put as variable, they will be interpreted as uncertain assignment to the group.
...
x <- rnorm(12)
linear_model_code <- nimbleCode({
  for(i in 1:3) #For loop to index the integer representation of the groups 
...
Somehow make a matrix of dummy variables. I'll do it with `model.matrix`. In this example, removing the implicit intercept `... -1`
...
```{r, eval=recalculate}
#Standard model matrix way of writing models
linear_model_code <- nimbleCode({
  for(i in 1:4)
    beta[i] ~ dnorm(0, sd = 100) # vague priors, the only line used in the for loop
  sigma ~ dunif(min = 0, max = 100)
...
Option 2 may be more useful in an instance with many factor level variables since it is simple to create a model matrix. However, this will come with the computational tradeoff.
...
Here is a **centered** version of the above example. Has the intercept as that mean.
...
And here is a fully uncentered version. (The first version might be called "partially" centered.) This can lead to good mixing in Hamiltonian Monte-Carlo (knowledge from the
