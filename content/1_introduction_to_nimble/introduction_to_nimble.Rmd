...
-   Laplace approximation and adaptive Gauss-Hermite quadrature (for maximum likelihood, included in *nimble* for now, likely to be moved to separate package later). Used in lme4 and used in tmb (template model builder) package. May also be Gaussian Quadrature
-   Coming soon: methods related to Integrated Nested Laplace Approximation (INLA). Not currently in NIMBLE, but coming soon!
-   Monte Carlo Expectation Maximization (MCEM, for maximum likelihood) (included in *nimble*).
-   Reversible Jump MCMC (RJMCMC) for variable selection (included in *nimble*). Used for moving between models and variable selection.
-   Marginal distributions for ecological models (capture-recapture, occupancy, dynamic occupancy, N-mixture, Hidden Markov models) (package *nimbleEcology*). Summing and integrating over discreet latent states.
...
Use simple models to introduce and work on concepts for the model language.
...
Section10p4_code <- nimbleCode({
  ...
  for (i in 1:M) {
    ...
    z[i] ~ dbern(psi[i])      # True (latent) occupancy z at site i
    logit(psi[i]) <- beta0 + beta1 * vegHt[i] #Deterministically Set
    for (j in 1:J) {
      # Observation model for the actual observations
      y[i,j] ~ dbern(p.eff[i,j])    # Detection-nondetection at site i and visit j
      p.eff[i,j] <- z[i] * p[i,j]   # 'straw man' for WinBUGS
      logit(p[i,j]) <- alpha0 + alpha1 * wind[i,j] #Deterministically set
    }
  }
  # Derived quantities are removed.
}
...
results <- nimbleMCMC(Section10p4_code,
                       ...
                       nburnin = 1000,
                       nchains = 2,
                       samplesAsCodaMCMC = TRUE, #Package from Mark and Plumber which is used to analyze JAGS outputs
                       WAIC = TRUE)
...
# Some key points about NIMBLE's model language (slides for each)
...
A node is something that appears on the left hand side of a declaration. Could be beta1. A variable contains a set of nodes. The probability (or more usually log probability) distribution on a stochastic declaration is used to determine the space and probability of values that a variable (or set of nodes) can take.

Variables/vectors that are not defined stochastically or deterministically within model code need to be provided in constants or data.

Nodes are what have the statistical meaning. Multivariate normal random effect, vector of transitions between states where the probabilities must sum to 1 (Dirichlet).

-   **How is this translated to hidden markov model?**

-   **Could a matrix bet defined as a node to assign probabilities to any pairwise state change, or is this represented as a vector?**
...
code <- nimbleCode({
  ...
  for(i in 1:8) {
    y[1:5, i] ~ ddirch(alpha[1:5]) #Dirichlet distribution draws an entire vector. Alphs is not defined in the model code, and therefore will need to come from either data or constants
  }
...
mymod <- nimbleCode({
  ...
  #Walk params
  mu ~dnorm(0, sd =100) #This is the prior distribution that the mean is drawn from
  sigma ~ dhalfflat() #prior for the standarad devistion (forces 0 to infinity and is an improper prior)
  
  for (i in 1:10) { #only 10 draws, could make it 1:N if N is in the setup code, or data
    x[i] ~ dnorm(mu, sd = sigma) 
  }
})
```
...
-   For backwards compatability with JAGS, not recommended when building fresh.
...
-   If parameter naming is not provided, defaults to BUGS/JAGS. Generally best to name parameters
...
-   NIMBLE names lifted nodes as a the relationship in text form:
...
occCode2 <- nimbleCode({
  # Priors
  mean.p ~ dunif(0, 1)         # Detection intercept on prob. scale
  alpha0 <- logit(mean.p)      # Detection intercept, deterministic node
  alpha1 ~ dunif(-20, 20)      # Detection slope on wind
  mean.psi ~ dunif(0, 1)       # Occupancy intercept on prob. scale
  beta0 <- logit(mean.psi)     # Occupancy intercept, deterministic
  beta1 ~ dunif(-20, 20)       # Occupancy slope on vegHt
