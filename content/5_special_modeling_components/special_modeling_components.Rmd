...
So, you have spatial data and need to account for the spatial autocorrelation that cannot be explained by covariates. Need a random effect for each spatial location and these REs need to be correlated. CAR model makes a valid spatial model that is set up to sample one location at a time for MCMC.

X[i]: x at location i
x[-i] : x at all other locations

Intuitively expressed as one-at-a-time conditionals:

$$ x_i|x_{-i} \sim N\left(\frac{1}{n_i} \sum_{j\in N_i} x_j, \frac{1}{n_i \tau}\right) $$

This ends up being a cyclic graph, and we think that we can't have it work, but some really smart people have made it work.
...
This is an **improper prior** because it does not integrate to one. (D half flat is also an improper prior)
...
- Modeled mean of the Poisson counts includes an offset for the expected count (e.g. population size). Has a random effect on the log scale
...
code <- nimbleCode({
  ...
  # likelihood
  for(i in 1:N) {
    lambda[i] <- expected[i] * exp(beta*z[i] + x[i]) #z[i] is the explanatory variable, expected is likely the population for each region
    y[i] ~ dpois(lambda[i])
  }
})
...
The presence of $\rho$ causes the prior to be proper (under certain constraints on $\rho$). This makes it a stationary, spatial, random process.
...
Mixtures of normal distributions can produce a very wide range of distributions. Two flavors: non-parametric tests & a model fitting way to fit an arbitrary line through things. An arbitrary mixture of normal distributions can present an extremely flexible way to fit non-parametric distributions.
...
We want to estimate these. To draw from the non-parametric distributions, we can randomly select which of the mixture pieces we should draw from, and then draw from it.
...
Methods are often related to a "Dirichlet process" (DP) (not the same as a Dirichlet distribution). (Deer-ish-lay)
...
- Each table represents a normal distribution in the mixture. Tables with more people are likely to attract the new individuals.
...
- These are the conjugate prior of the Dirichlet distribion and the multinomial as N appraoches infinity.
...
```{r, eval=recalculate}
code <- nimbleCode({
  z[1:N] ~ dCRP(alpha, size = N) # Follows the chineese restaurant process.Alpha is a concentration parameter, the larger alpha is, the more clustered things are.
  alpha ~ dgamma(1, 1)
  for(i in 1:M) {
    thetatilde[i] ~ dnorm(0, var = 100)
    s2tilde[i] ~ dinvgamma(1, 1) #S squared tilde, conjugate prior for a variance
  }
  for(i in 1:N) 
...
It can be difficult to see the BNP distribution itself. There is a tool for that: (It will use `compileNimble` and take a minute.) This accesses the 

```{r, eval = recalculate}
DPsamples <- getSamplesDPmeasure(comp$mcmc)
```

`DPsamples` is a *list* matrices, one for each row of the MCMC samples. Let's look at a few. This gives us the means and variances for the different normal distributions that were used/estimated for each MCMC pull to characterize the non-parametric approach.
