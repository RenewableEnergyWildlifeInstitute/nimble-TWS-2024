...
library(nimble)
lmCode <- nimbleCode({
  psi ~ dunif(0,1)   # prior on inclusion probability, probability the variable is included in the model
  sigma ~ dunif(0, 20)
  for(i in 1:numVars) {
  ...
  for(i in 1:n) {
    pred.y[i] <- inprod(X[i, 1:numVars], zbeta[1:numVars]) #Matrix multiplication, one row at a time,
...
Simulating data within the model itself is a major benefit to the NIMBLE approach since it ensures consistency
```{r}
...
See how the model is just following it's prior. There is a lot of variance in the estimate itself, check the y-axis
```{r}
...
### Wasteful computation (a secondary issue and curse word)
...
- Proposing, accepting, and rejecting
...
- Make a model with all coefficients, but use the RJMCMC ot treat is as though we are reducing the model and removing coefficients 

RJMCMC for variable selection in nimble
=====

- Update an MCMC configuration to use RJMCMC.
- If indicator is 1, propose to remove the coefficient from the model and ignore until, to put a coeffieicient back into the model, there needs to be a proposal for a non-0 indicator variable
