...
We are interested in the joint posterior distribution of the unknown variables (parameters and states). Parameters are random variables. Get the distribution of the data from integrating over the parameters and states. MCMC is a bit of a brute-force integration method. See more below.
...
- Wikipedia has a really good conjugate prior page/list. 
...
Workhorse if you don't have conjugate sampling distributions. Propose a new value, and then jump proportional to the gains.
...
If you want any customization, use the 4-step process. The configureMCMC is the recipe for setting up samplers. To configure the sampler for given nodes, you would assign them after the buildMCMC step. 
...
=====
When you create a NIMBLE model, it is an object in R. This is a dynamically generated S3 object. 
...
"Dependencies" (or *dependents* or *children*) of "x'" are the nodes that depend on "x" (including itself), recursively, stopping at stochastic nodes.Can be helpful in understanding nimble code and nimble objects. Can use to explore what depends on a given parameter.
...
You want to make sure that it runs fast, but also want the chains to mix.
...
- House martin abundance estimates from 1990-2009, includes the effect of previous abundance estimate
...
martin_code <- nimbleCode({
  ...
  logN.est[1] ~ dnorm(5.6, 0.01)    # Prior for initial population, true state of population that is unknown. Prior required for the first state because there is no prior state to the measured first.
  ...
  # Population sizes on real scale, not really part of the model. This is a derived parameter. Deriving parameters can during the MCMC process can slow down the model compared to calculating a single time, post-fitting.
  for (t in 1:T) {
...
1. Process-noises are random variables (above).  States are deterministic given process noises. 
2. States are random variables which allows for less dependency (below).
...
Rewrite the model
=====
Here is a more efficient way to write a state-space model. This is statistically equivalent, but since the we incorporate the random noise within the logN.est variable (and therefore all logN.est nodes) rather than adding it after the node is estimated. This may also change the mixing.
...
We can use these functions to look at the metrics associated with the a given model run.
...
- categorical (these are *costly*) for latent states that take integer values
...
There are multiple thinning options for when there are large mismatches in the number of covariates vs latent states.
