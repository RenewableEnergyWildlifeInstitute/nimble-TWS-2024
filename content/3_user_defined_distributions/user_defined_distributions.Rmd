...
Example where we marginilize a variable to get a summed probability of another. We can start by writing the model that we want to be able to do. 
...
Almost everything in nimble can be run in R (albeit very slowly). This can be useful for debugging and ensuring we are getting things right! Something pretty neat about occupancy modeling and capture recapture where latent state is utilized, the observations of 1 can be provided to z[i] so that the sampler see it as data and not as a space which needs to be explored.

* Write dOcc (`dOcc_R`) as an R function: get our math and logic right.
* Test/debug `dOcc_R`.
* Convert `dOcc_R` to a `nimbleFunction` (`dOcc`). It will look *almost* identical
...
Writing an extension function needs to have a `log=...` parameter that flags whether prob or logprob should be returned. When called from a model, it will default to an assumed true, but can be built out for both for be general. 
...
What is a `nimbleFunction`?

- Can be used in a model or algorithm
- Can use models (e.g. an MCMC sampler) if it has "`setup`" code (not covered). More advanced version.
- Can be compiled.
- "compiled" means that nimble will generate C++, compile that, and make it available for use from R.
- Supports much of R's math, distributions, and basic flow control (for loops, if/then/else).
- Does not support objects or functions of any type complexity (lists, environments, lapply)
- Requires type annotation for inputs and outputs (scalar, vector, matrix, array). 0 index dimensions is a scalar (single number), 1 index dimension will be vector, 2: matrix, 3: 3-dimensional array, etc.

```{r}
dOcc <- nimbleFunction(
  run = function(x = double(1), # argument type declarations, double precision with 1 index dimension (vector)
                 probOcc = double(0),
                 probDetect = double(1),
    ...
    if (log)
      return(log(prob_x))
    return(prob_x) # Need to be explicit about what the return is, required
  }
)
```
...
Debugging a function can use the browser to step through the target function if an error is caught. `Debug()` assigns a state to the function and can be turned off, `debugonce()` will only run it during the next call.
...
Test to make sure it actually works by assigning values to detection probabilities (p) and latent probability (psi).
...
We can run just a few MCMC iterations within R ensure that the compiled function will work when it hits the MCMC section.
...
* `nimbleEcology` provides different versions (`dOcc_v` - Detection probability is a vector, `dOcc_s` - Detction probability is a scalar) for time-varying vs time-constant survival and capture probabilities.
* More error trapping
